...
export const Preloader: React.FC<PreloaderProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="absolute w-96 h-96 rounded-full bg-sky-500/20 blur-[100px] pointer-events-none animate-pulse" />
      <div className="absolute w-80 h-80 rounded-full bg-sky-400/15 blur-[90px] pointer-events-none" />

      {/* Main Animated Logo Box */}
      <div className="relative flex flex-col items-center gap-6 z-10">
        <div className="relative">
          {/* Pulsing Outer Tech Rings */}
          <div className="absolute -inset-4 rounded-3xl bg-gradient-to-r from-sky-400 via-sky-500 to-blue-600 opacity-40 blur-lg animate-pulse" />
          <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-slate-900 border border-slate-700/80 p-0.5 shadow-2xl flex items-center justify-center overflow-hidden">
            ...
            <div className="relative flex items-center justify-center text-white">
              <GraduationCap className="w-10 h-10 sm:w-12 sm:h-12 text-sky-400 animate-bounce" />
              <Sparkles className="w-4 h-4 text-sky-200 absolute -top-1 -right-1 animate-spin" style={{ animationDuration: '3s' }} />
            </div>

            {/* Dynamic Orbit Light Line */}
            <div className="absolute inset-x-0 h-0.5 bg-gradient-to-r from-transparent via-sky-400 to-transparent animate-pulse" />
          </div>
        </div>
        ...
        <div className="text-center space-y-1">
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-2xl sm:text-3xl font-black text-sky-400 tracking-wider">ITECH</span>
        ...
        <div className="w-64 sm:w-72 space-y-2 mt-2">
          <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden p-0.5 border border-slate-700/50">
            <div
              className="h-full bg-gradient-to-r from-sky-400 via-sky-500 to-blue-500 rounded-full transition-all duration-200 ease-out"
              style={{ width: `${Math.min(100, progress)}%` }}
            />
          </div>
          <div className="flex items-center justify-between text-[10px] text-slate-400 font-mono">
            <span className="flex items-center gap-1">
              <Cpu className="w-3 h-3 text-sky-400 animate-spin" style={{ animationDuration: '4s' }} />
              Plateforme & Tuteur IA
            </span>
            <span className="text-sky-300 font-bold">{Math.min(100, progress)}%</span>
          </div>
        </div>
      </div>
    </div>
